...
import static ru.netology.dsw.processor.HWAlertsApp.STATE_STORE_NAME2;

public class HWAlertTrasformer implements Processor<String, GenericRecord, String, GenericRecord> {
    ...
    public void init(ProcessorContext<String, GenericRecord> context) {
        stateStore = context.getStateStore(STATE_STORE_NAME2);
    ...
    public void process(Record<String, GenericRecord> record) {
        ...
        long newVal = oldVal != null ? oldVal + income : income;
    ...
    private void sendAlerts(long timestamp) {
        ...
        stateStore.range(createKey(lastProcessedWindowEnd, ""), createKey(nearestMinutesTs, ""))
                .forEachRemaining(keyVal -> {
                    ...
                    Long income = keyVal.value;
                    if (income > HWAlertsApp.MAX_INCOME_PER_MINUTE) {
